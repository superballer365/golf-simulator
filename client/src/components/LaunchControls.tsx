...
import {
    SimulationStatus,
    useLaunchConditions, useSimulationActions, useSimulationStatus,
...
export default function LaunchControls() {
    ...
    const { displayToStorageUnit: angleDisplayToStorageUnit, storageToDisplayUnit: angleStorageToDisplayUnit } 
        = useUnitHelper(MeasurementTypes.Angle);
    const { displayToStorageUnit: speedDisplayToStorageUnit, storageToDisplayUnit: speedStorageToDisplayUnit }
        = useUnitHelper(MeasurementTypes.Speed)
    const { start, reset } = useSimulationActions();

    const roundedInitialSpeed = speedStorageToDisplayUnit(launchConditions.speed).toFixed(2)
    const roundedInitialVerticalAngle = angleStorageToDisplayUnit(launchConditions.verticalAngle).toFixed(2)
    const [speed, setSpeed] = useState(roundedInitialSpeed)
    const [verticalAngle, setVerticalAngle] = useState(roundedInitialVerticalAngle)
