	...
	actions: {
		start: (launchConditions?: Partial<LaunchConditions>) =>
			set((state) => {
				...
				console.log('setting launch conditions to: ', JSON.stringify(launchConditions))
				return {
		...
		advanceSimulation: (by: number) =>
			set((state) => {
				...
				const newXPosition = xOt(
					newElapsedTime,
					0,
					Math.cos(state.launchConditions.verticalAngle) * state.launchConditions.speed
				)
				const newYPosition = yOt(
					newElapsedTime,
					0,
					Math.sin(state.launchConditions.verticalAngle) * state.launchConditions.speed
				)
				...
				const newStatus = newPosition.y <= 0 ? SimulationStatus.Complete : state.status
				return {
					status: newStatus,
...
export const useBallPosition = () => useSimulationStore((state) => state.ballPosition)
export const useLaunchConditions = () => useSimulationStore((state) => state.launchConditions)
